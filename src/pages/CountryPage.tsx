...
import { Breadcrumb, generateCountryBreadcrumbs } from "@/components/Breadcrumb";
import { MovieCardSkeleton } from "@/components/MovieCardSkeleton";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { getTMDBMoviesByCountry, TMDBMovie, getTMDBImageUrl, getGenreNames, COUNTRY_LIST } from "@/lib/external-api";
...
const CountryPage = () => {
  ...
  const country = COUNTRY_LIST.find((c) => c.code.toLowerCase() === code?.toLowerCase());
  const countryName = country?.name || code?.toUpperCase();

  const { data: moviesData, isLoading } = useQuery({
    queryKey: ["tmdb-movies-country", code, page],
    queryFn: () => getTMDBMoviesByCountry(code!.toUpperCase(), page),
    enabled: !!code,
  });
  ...
  const seoTitle = `Film ${countryName} - Film Dong`;
  const seoDescription = `Nonton film dari ${countryName} dengan subtitle Indonesia. Koleksi lengkap film ${countryName} berkualitas HD di Film Dong.`;
  ...
  return (
    <Layout>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Breadcrumb items={generateCountryBreadcrumbs(countryName || "")} className="mb-6" />
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Browse all movies from {countryName}</p>
            </div>
          </div>
        </div>
        ...
        {!isLoading && movies.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground mb-6">
              No movies found from {countryName}.
            </p>
            <Button asChild>
